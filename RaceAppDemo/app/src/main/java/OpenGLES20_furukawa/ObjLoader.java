...
public class ObjLoader {
    ...
    public static Figure load(String path) throws Exception {
        /*//DataInputStream in=new DataInputStream(GLES.context.getAssets().open(path));
        File file = new File(path);
        BufferedReader br = new BufferedReader(new FileReader(file));*/
        DataInputStream in=new DataInputStream(GLES.context.getAssets().open(path));
        int counter = 0;//@TODO debug parameter.

        //File file = new File(GLES.context.getAssets().open(path));
        //BufferedReader br = new BufferedReader(new FileReader(file));
        ...
        try {
            while (line != null) {
                ...
                //Log.i("couter_Param", c.toString());
            }
        }catch (NumberFormatException e){
            Log.e("debug", "counter " + counter);
        }
        //in.close();
        br.close();
        //@TODO Test msg.
        Log.i("File load", "Successful!");
